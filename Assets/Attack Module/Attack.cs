...
public class FreezeEffect : MonoBehaviour
{
    ...
    private bool usedBehaviorFreeze = false;
    private bool usedRigidbodyFreeze = false;
    ...
    public void StartFreeze(float duration)
    {
        ...
        // 方案A：如果敌人有 EnemyAI，用行为冻结
        if (enemyAI != null)
        {
            enemyAI.SetFrozen(true);
            usedBehaviorFreeze = true;
            Debug.Log($" 通过 EnemyAI 冻结 {name}");
        }
        // 方案B：否则尝试用 Rigidbody2D 冻结
        else if (rb != null)
        {
            originalVelocity = rb.velocity;
            rb.velocity = Vector2.zero;
            rb.isKinematic = true;
            usedRigidbodyFreeze = true;
            Debug.Log($" 通过 Rigidbody2D 冻结 {name}");
        }
        else
        {
            Debug.LogWarning($" {name} 无法冻结：既无 EnemyAI 也无 Rigidbody2D");
            Destroy(this);
            return;
        }
    ...
    private IEnumerator FreezeCoroutine()
    {
        ...
        if (usedBehaviorFreeze && enemyAI != null)
        {
            enemyAI.SetFrozen(false);
        }
        else if (usedRigidbodyFreeze && rb != null)
        {
